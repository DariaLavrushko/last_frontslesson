...
const TodoList: React.FC = (): JSX.Element => {
    ...
    // Единственный параметр метода setCheck является предыдущее значение check
    // setCheck((prev) => !prev);

    // setCheck((prev) => !prev); // Изменяем check на обратное значение
    // setCheck1(!check);
    ...
    const handleRemoveTask = (index: number): void => {
        // оператор spread, позволяет создать глубокую копию массиву
    ...
    const handleIsDone = (e: ChangeEvent<HTMLInputElement>) => {
        // в переменную check кладём актуальное значение поля checked у event.target
                ...
                {tasksList.map((task, index) => (
                    ...
                    */
                    <div key={index + 1} className="task">
                        <li style={task.completed ? { backgroundColor: 'green' } : {backgroundColor: 'yellow'}}>
                            <span>{index + 1}. </span>
                            {task.name}
                            <input
                                type="checkbox"
